/*!
 **|   MediaSync Audio Notifications System
 **|   Copyright 2017 MM
 **|   Version 2017.11.12.
 **|
 **|  
...
if (!window[windowChannelIndex]) {
    ...
    if (typeof Storage === "undefined") {
        console.error("[MSTK: Audio Notifier]", "localStorage not supported. Aborting load.");
        return
    } else if (typeof window.playlist !== "function") {
        console.error("[MSTK: Audio Notifier]", "Playlist parser unavailable. Aborting load.");
        return;
        return
    } else {
        console.info("[MSTK: Audio Notifier]", "Loading Module.")
    }
    if (typeof window[windowChannelIndex].audioLibrary === "undefined") {
        console.warn("[MSTK: Audio Notifier]", "WARNING: Audio library module not loaded.")
    }
    if (!$("#customSettingsStaging").length) {
        console.warn("[MSTK: Audio Notifier]", "WARNING: Settings module not loaded.")
    }
    const AudioNotifier = function() {
        ...
        this.choices = Object.assign({}, {
            Frank_Hyukaoee: "http://k003.kiwi6.com/hotlink/caywn1tkmv/frank-hyukaoee.mp3",
            ThisWontTakeLong: "http://k003.kiwi6.com/hotlink/zs3q56219a/crow-thiswonttakelong.mp3",
            Crow_SexFilm: "http://k003.kiwi6.com/hotlink/68rfv4pyww/crow-sexfilm.mp3",
            DoesThisBugYou: "http://k003.kiwi6.com/hotlink/k4778w5qrl/mst3k-doesthisbugyou.mp3",
            Huzzah: "http://k003.kiwi6.com/hotlink/47q7925sdh/mst3k-huzzah.mp3",
            PushTheButtonFrank: "https://www.mst3ktemple.com/sounds/catch-pushbutton.wav",
            Hikeeba: "https://www.mst3ktemple.com/sounds/catch-hikeeba.wav",
            Poopie: "https://www.mst3ktemple.com/sounds/tm-poopie.wav",
            JoinUs: "https://www.mst3ktemple.com/sounds/2/207-joinus.wav",
            TorgoTheme: "https://www.mst3ktemple.com/sounds/4/424-torgotheme.wav"
        ...
        this.handler = {
            ...
            Poll: function(data) {
                ...
                if (Date.now() - this.Poll.timeSinceLast < 36e4) return;
    ...
    Object.assign(AudioNotifier.prototype, {
        ...
        initialize: function() {
            ...
            if (window[windowChannelIndex].modulesOptions && window[windowChannelIndex].modulesOptions.audioNotice) {
                this.choices = Object.assign(this.choices, window[windowChannelIndex].modulesOptions.audioNotice.choices);
                var notices = Object.keys(window[windowChannelIndex].modulesOptions.audioNotice.notices);
                for (var i = notices.length - 1; i >= 0; i--) {
                    this[notices[i]]["id"] = window[windowChannelIndex].modulesOptions.audioNotice.notices[notices[i]]
                }
            }
    ...
    window[windowChannelIndex].audioNotice = (new AudioNotifier).initialize()
});
